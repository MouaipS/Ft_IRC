...
static bool isAvailable(std::string name, User *user, std::map<int, User*>& fdToUser) {
    if(user->isNicknameValid(name) == true) {
...
void CmdNick::execCmd(
    ...
    if(isAvailable(cmd[1], fdToUser[fd_origin], fdToUser) == false) {  
        ...
        std::string message = ":";
        message = message + servername + " 461 " + fdToUser[fd_origin]->getNickname() + " :Not enough parameters" + fdToUser[fd_origin]->getNickname() + "\r\n";
        std::cout << "test 1" << std::endl;
        ICommand::sendToUser(fd_origin, message, 0);
    }
    else if (ICommand::isUserValidAuth(*fdToUser[fd_origin], 1, 0, 1)) {
        //PASS 1 NICK 0 USER 1 -> Code "001" MSG ":Welcome to the IRC Network"
        fdToUser[fd_origin]->setNickname(cmd[1]);
        std::string message = ":";
        message = message + servername + " 001 " + servername + " :Welcome to the IRC Network " + fdToUser[fd_origin]->getNickname() + "\r\n";  //001 RPL_WELCOME
        ...
        std::string message = ":";
        message = message + servername + " 464 " + fdToUser[fd_origin]->getNickname() + " :NERR_PASSWDMISMATCH" + fdToUser[fd_origin]->getNickname() + "\r\n";
        std::cout << "test 3" << std::endl;
        ICommand::sendToUser(fd_origin, message, 0);
    }
    else {
        //PASS 1 NICK 0 USER 0 -> RIEN (ON CHANGE SIMPLEMENT NICKAME)
        fdToUser[fd_origin]->setNickname(cmd[1]);
